...
public enum DesktopCommandRunner {
        ...
        stdout: OutputHandler) throws
    {
        ...
        case "get-grid-item", "getGridItem":
            try self.runAutomationGetGridItem(args: args, adapter: adapter, stdout: stdout)
        case "move":
    ...
    private static func runAutomationFocus(
        args: [String],
        adapter: any DesktopAdapter,
        stdout: OutputHandler) throws
    {
        let indexValue = try self.value(after: "--index", in: args) ??
            self.value(after: "--element-index", in: args)
        guard let indexValue else {
            throw DesktopAdapterError.invalidArgument("Missing --index <element-index> for automation focus")
        }
        ...
        try stdout(self.success(adapter.focusUIAutomationElement(
            scope: scope,
            maxDepth: maxDepth,
            maxElements: maxElements,
            elementIndex: self.parseUIAutomationElementIndex(indexValue))))
    ...
    private static func runAutomationLegacyDefaultAction(
        args: [String],
        adapter: any DesktopAdapter,
        stdout: OutputHandler) throws
    {
        ...
        try stdout(self.success(adapter.performUIAutomationElementLegacyDefaultAction(
            scope: scope,
            maxDepth: maxDepth,
            maxElements: maxElements,
            elementIndex: self.parseUIAutomationElementIndex(indexValue))))
    ...
    private static func runAutomationSetZoom(
        args: [String],
        adapter: any DesktopAdapter,
        stdout: OutputHandler) throws
    {
        ...
        guard let indexValue else {
            throw DesktopAdapterError.invalidArgument(
                "Missing --index <element-index> for automation set-zoom")
        }

        let zoomLevelValue = try self.value(after: "--level", in: args) ??
            self.value(after: "--zoom-level", in: args) ??
            self.value(after: "--value", in: args)
        guard let zoomLevelValue else {
            throw DesktopAdapterError.invalidArgument(
                "Missing --level <percent> for automation set-zoom")
        }
        ...
        try stdout(self.success(adapter.setUIAutomationElementZoomLevel(
            scope: scope,
            maxDepth: maxDepth,
            maxElements: maxElements,
            elementIndex: self.parseUIAutomationElementIndex(indexValue),
        ...
        stdout: OutputHandler) throws
    {
        ...
        guard let indexValue else {
            throw DesktopAdapterError.invalidArgument(
                "Missing --index <element-index> for automation get-grid-item")
        }

        let rowValue = try self.value(after: "--row", in: args)
        guard let rowValue else {
            throw DesktopAdapterError.invalidArgument("Missing --row <row> for automation get-grid-item")
        }

        let columnValue = try self.value(after: "--column", in: args) ??
            self.value(after: "--col", in: args)
        guard let columnValue else {
            throw DesktopAdapterError.invalidArgument("Missing --column <column> for automation get-grid-item")
        }
        ...
        try stdout(self.success(adapter.getUIAutomationGridItem(
            scope: scope,
            maxDepth: maxDepth,
            maxElements: maxElements,
            elementIndex: self.parseUIAutomationElementIndex(indexValue),
            row: self.parseUIAutomationGridCoordinate(rowValue, label: "row"),
            column: self.parseUIAutomationGridCoordinate(columnValue, label: "column"))))
    }
    ...
    private static func parseUIAutomationGridCoordinate(_ value: String, label: String) throws -> Int {
        guard let coordinate = Int(value), (0...Int(Int32.max)).contains(coordinate) else {
            throw DesktopAdapterError.invalidArgument(
                "UI Automation grid \(label) must be a non-negative integer")
        }
        return coordinate
    }
    ...
    private static func helpText(commandName: String, adapter: any DesktopAdapter) -> String {
        ...
        Commands:
          ...
          automation get-spreadsheet-item --index <n> --name <cell-name>
            [--scope root|foreground|focused|cursor] [--max-depth <n>] [--max-elements <n>]
          automation get-grid-item --index <n> --row <row> --column <column>
            [--scope root|foreground|focused|cursor] [--max-depth <n>] [--max-elements <n>]
